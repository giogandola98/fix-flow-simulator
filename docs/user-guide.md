...
**Tip:** Paste a captured FIX message from a log file using the **Paste FIX Message** button. The parser strips header/trailer tags automatically. If the message contains a repeating group whose counter tag the parser recognises, it is reconstructed into the Repeating Groups panel below; a counter tag it does not recognise is left as flat fields and the parser reports it in a warning so you can build that group by hand.
